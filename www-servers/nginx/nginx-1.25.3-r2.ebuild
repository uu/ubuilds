...
ZIP_MODULE_PV="1.3.0"
ZIP_MODULE_P="zip-${ZIP_MODULE_PV}"
ZIP_MODULE_URI="https://github.com/evanmiller/mod_zip/archive/refs/tags/${ZIP_MODULE_PV}.tar.gz"
ZIP_MODULE_WD="${WORKDIR}/mod_zip-${ZIP_MODULE_PV}"
...
SRC_URI="https://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_zip? ( ${ZIP_MODULE_URI} -> ${ZIP_MODULE_P}.tar.gz )
...
src_configure() {
	...
	if use nginx_modules_http_zip; then
		http_enabled=1
		myconf+=( --add-module=${ZIP_MODULE_WD} )
	fi
