...
HTTP_REDIS_MODULE_PV="0.08rc1"
HTTP_REDIS_MODULE_P="ngx_redis-${HTTP_REDIS_MODULE_PV}"
HTTP_REDIS_MODULE_SHA1="23cf589"
HTTP_REDIS_MODULE_URI="http://github.com/agentzh/redis2-nginx-module/tarball/v${HTTP_REDIS_MODULE_PV}"

# http_set_mix (https://github.com/agentzh/set-misc-nginx-module, BSD license)
HTTP_SET_MIX_MODULE_PV="0.22rc3"
HTTP_SET_MIX_MODULE_P="ngx_set_mix-${HTTP_SET_MIX_MODULE_PV}"
HTTP_SET_MIX_MODULE_SHA1="7adef5a"
HTTP_SET_MIX_MODULE_URI="http://github.com/agentzh/set-misc-nginx-module/tarball/v${HTTP_SET_MIX_MODULE_PV}"

# http_devel_kit (https://github.com/simpl/ngx_devel_kit, BSD license)
HTTP_DEVEL_KIT_MODULE_PV="0.2.17rc2"
HTTP_DEVEL_KIT_MODULE_P="ngx_devel_kit-${HTTP_DEVEL_KIT_MODULE_PV}"
HTTP_DEVEL_KIT_MODULE_SHA1="bc97eea"
HTTP_DEVEL_KIT_MODULE_URI="http://nodeload.github.com/simpl/ngx_devel_kit/tarball/v${HTTP_DEVEL_KIT_MODULE_PV}"
...
SRC_URI="http://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_set_mix? ( ${HTTP_SET_MIX_MODULE_URI} -> ${HTTP_SET_MIX_MODULE_P}.tar.gz )
...
CDEPEND="
	...
	nginx_modules_http_lua? ( dev-lang/lua )
	nginx_modules_http_redis? ( dev-db/redis )
...
src_configure() {
	...
	if use nginx_modules_http_devel_kit; then
		http_enabled=1
		myconf+=" --add-module=${WORKDIR}/simpl-ngx_devel_kit-${HTTP_DEVEL_KIT_MODULE_SHA1}"
	fi
	if use nginx_modules_http_redis; then
		http_enabled=1
		myconf+=" --add-module=${WORKDIR}/agentzh-redis2-nginx-module-${HTTP_REDIS_MODULE_SHA1}"
	fi
	if use nginx_modules_http_set_mix; then
		http_enabled=1
		myconf+="
		--add-module=${WORKDIR}/agentzh-set-misc-nginx-module-${HTTP_SET_MIX_MODULE_SHA1}"
	fi
