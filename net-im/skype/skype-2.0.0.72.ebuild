...
built_with_use ">=x11-libs/qt-4.0" accessibility dbus ; then
			eerror "Rebuild qt-4 with USE=\"accessibility 
dbus\" or set USE=qt-static"
			die "USE=\"-qt-static\" only works with qt-4 
built with USE=\"accessibility dbus\"."
		elif has_version '>=x11-libs/qt-4.4.0_alpha:4' && ! 
built_with_use ">=x11-libs/qt-gui-4.4.0:4" accessibility dbus ; then
                        eerror "Rebuild qt-gui-4 with 
USE=\"accessibility dbus\" or set USE=qt-static"
                        die "USE=\"-qt-static\" only works with qt-gui-4 
built with USE=\"accessibility dbus\"."
		fi
	fi
}
...
src_install() {
	...
	make_wrapper skype /opt/${PN}/skype /opt/${PN} /opt/${PN} 
/usr/bin
	...
	insinto /opt/${PN}/lang
	#
	#There have been some issues were lang is not updated from the 
.ts files
	...
	# doins skype.desktop
	make_desktop_entry ${PN} "Skype VoIP" ${PN} 
"Network;InstantMessaging;Telephony"

	#Fix for no sound notifications
	dosym /opt/${PN} /usr/share/${PN}

	# TODO: Optional configuration of callto:// in KDE, Mozilla and 
friends
...
pkg_postinst() {
	...
	elog "-- Preemption Model (Preemptible Kernel (Low-Latency 
Desktop))"
	elog "-- Timer frequency (250 HZ)"
	elog
	ewarn "This release no longer uses the old wrapper because ${PN} 
now uses ALSA"
	ewarn
