...
src_install() {
	...
	newinitd "${FILESDIR}/${PN}.initd" ucarp || die
	newconfd "${FILESDIR}/${PN}.confd" ucarp || die
	dodir /etc/${PN}
	insinto /etc/${PN}	
	insopts -m0744
	doins "${FILESDIR}/vip-up.sh" 
	doins "${FILESDIR}/vip-down.sh" 
	dodoc "${DOCS[@]}"
	doman "${FILESDIR}"/ucarp.8
	einfo "Do not forget to edit /etc/conf.d/ucarp!"
}
