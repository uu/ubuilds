...
SLOT="6.0"
KEYWORDS="~x86"
COMMON_DEPEND="=dev-util/netbeans-openide-modules-${SLOT}*"
...
src_unpack() {
	...
	java-pkg_jar-from --into ${S}/nbbuild/netbeans/platform7/lib netbeans-openide-modules-${SLOT}
}

src_compile() {
	ANT_TASKS="netbeans-nbbuild-6.0 ant-nodeps" eant -f libs/jsch/build.xml netbeans -Ddo-not-rebuild-clusters=true
}

src_install() {
	java-pkg_dojar nbbuild/netbeans/ide8/modules/org-netbeans-libs-jsch.jar
	insinto /usr/share/${PN}-${SLOT}/lib/config
	doins nbbuild/netbeans/ide8/config/Modules/org-netbeans-libs-jsch.xml
	insinto /usr/share/${PN}-${SLOT}/lib/update_tracking
	doins nbbuild/netbeans/ide8/update_tracking/org-netbeans-libs-jsch.xml
}
