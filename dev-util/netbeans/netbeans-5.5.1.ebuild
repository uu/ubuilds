...
FILE_MOBILITY="${PN}_mobility-${MY_PV}-linux.bin"
FILE_PROFILER="${PN}-profiler-${MY_PV}-linux.bin"
FILE_VISUALWEB="${PN}-visualweb-${MY_PV}-linux.bin"
SRC_URI="http://us1.mirror.netbeans.org/download/${MY_PV}/fcs/200704122300/${FILE_IDE}
	c++? ( http://us1.mirror.netbeans.org/download/${MY_PV}/cnd/fcs/070510/${FILE_CPP} )
	mobility? ( http://us1.mirror.netbeans.org/download/${MY_PV}/fcs/200704122300/${FILE_MOBILITY} )
	profiler? ( http://us1.mirror.netbeans.org/download/${MY_PV}/fcs/200704122300/${FILE_PROFILER} )
	visualweb? ( http://us1.mirror.netbeans.org/download/${MY_PV}/vwp/fcs/070516_2/${FILE_VISUALWEB} )"
LICENSE="CDDL"
SLOT="5.5"
KEYWORDS="~amd64 ~x86 ~x86-fbsd"
IUSE="c++ debug doc mobility profiler visualweb"
...
src_unpack () {
	...
	use mobility && unpack_extra ${FILE_MOBILITY} mobility
	use profiler && unpack_extra ${FILE_PROFILER} profiler
	use visualweb && unpack_extra ${FILE_VISUALWEB} visualweb
	cd ${S}
...
src_install() {
	...
	use c++ && install_extra cpp
	use mobility && install_extra mobility
	use profiler && install_extra profiler
	use visualweb && install_extra visualweb
