...
pkg_setup() {
	...
	if use amd64 ; then
		export ECLIPSEARC=x86_64
	else
		export ECLIPSEARC=${ARCH}
	fi

	if use x86 ; then
		export JVM_ARCH=i386
	else
		export JVM_ARCH=${ARCH}
	fi
}
...
src_compile() {
	...
	if [[ $(java-pkg_get-vm-vendor) == "ibm" ]] ; then
		...
		JAVA_LIB_DIR="$(java-config --jdk-home)/jre/lib/${JVM_ARCH}"
	fi
	...
	export DEFAULT_JAVA_JNI="$(java-config --jdk-home)"
	...
	local bootclasspath=$(GENTOO_VM=${java5vm} java-config --runtime)

	#einfo "Using ${java5home} for java5home"
	export JAVACFLAGS="-source 1.4 -target 1.4"
	ANT_OPTS=-Xmx1024M \
		#eant \
		#-Dnobootstrap=true \
		#-DinstallOs=linux -DinstallWs=gtk -DinstallArch=${ECLIPSEARC} \
		#-Dlibsconfig=true \
		#-Dbootclasspath=${bootclasspath} \
		#-Djava5.home=${java5home}
		eant -DinstallOs=linux -DinstallWs=gtk -DinstallArch=${ECLIPSEARC} \
		-Dlibsconfig=true -Dbootclasspath=${bootclasspath} \
		-DjavacVerbose=true
}
...
src_install() {
	...
	[[ -f result/linux-gtk-${ECLIPSEARC}-sdk.tar.gz ]] \
		|| die "tar.gz bundle was not built properly!"
	tar zxf result/linux-gtk-${ECLIPSEARC}-sdk.tar.gz -C ${D}/usr/lib \
		|| die "Failed to extract the built package"

	mv "${D}/usr/lib/eclipse" "${D}/${ECLIPSE_DIR}"
	
	# Instead of unpacking the build tar
	# Just cp the directory that was tar'd in the first place.
	#cp -R eclipse ${D}/${ECLIPSE_DIR} || die "Unable to copy eclipse dir"

	debug-print "Installing eclipse-gtk binary"
	exeinto ${ECLIPSE_DIR}
	doexe eclipse/eclipse || die "Failed to install eclipse binary"

	#Copy over missing library for eclipse
	cp launchertmp/library/gtk/*.so \
		"${D}/${ECLIPSE_DIR}/plugins/org.eclipse.equinox.launcher.gtk.linux.x86_64_1.0.0.v20070606/"
...
install-link-system-jars() {
	...
	rm lucene-1.9.1_*.jar
	java-pkg_jar-from lucene-1.9 lucene.jar lucene-1.9.1.v200706111724.jar
	popd > /dev/null
	## END LUCENE ##

	# Already a link to system jar :)
	#pushd plugins/org.junit4/ >/dev/null
	#rm junit.jar
	#java-pkg_jar-from junit-4 junit.jar
	#popd >/dev/null
...
patch-jni-libs() {
	...
	# %patch4 -p0
	# I don't believe this is required now :)
	#epatch ${WORKDIR}/${P}-libupdatebuild.patch
	# %patch5 -p0
	epatch ${WORKDIR}/eclipse-libupdatebuild2.patch

	# Build swttools.jar
	pushd plugins/org.eclipse.swt.gtk.linux.x86_64 >/dev/null || die "pushd failed"
	# %patch18 -p0
	epatch ${WORKDIR}/eclipse-swttools.patch
	popd >/dev/null

	#pushd plugins/org.eclipse.core.filesystem/natives/unix/linux >/dev/null || die "pushd failed"
	#mv Makefile Makefile.orig || die "Failed to move Makefile"
	#sed 's!JAVA_HOME=!JAVA_HOME=\"'"${JAVA_HOME}"'\"!' Makefile.orig > Makefile || die "sed failed"
	#popd >/dev/null
	# %patch22 -p0
	...
	# keep the old patch
	# ali_bush:  I have no idea if this is required still so lets try
	# without it
...
patch-tomcat() {
	pushd plugins/org.eclipse.tomcat >/dev/null || die "pushd failed"
	# %patch28 -p0
	epatch ${WORKDIR}/eclipse-tomcat55.patch
	# %patch29 -p0
	epatch ${WORKDIR}/eclipse-tomcat55-build.patch
	popd >/dev/null
...
patch-launcher() {
	...
	# ali_bush Added.
	epatch "${WORKDIR}/eclipse-launcher-double-free-bug.patch"
	
	pushd library/gtk > /dev/null || die "pushd failed"
	mv build.sh build.sh.bak
	sed 's_javaHome=\"\"_javaHome=\"'"${JAVA_HOME}"'\"_' build.sh.bak > build.sh || die "Unable to sed launcher build.sh"
	popd

	# %patch47 -p1
	#epatch ${WORKDIR}/${PN}-launcher-link.patch
...
patch-symlink-junit() {
	...
	pushd plugins/org.junit_*/ > /dev/null || die "pushd failed"
...
patch-symlink-jsch() {
	...
	# This is no there so no need for it?
...
patch-apply-all() {
	...
	# TODO figure out what ${P}-bz162177.patch is and if we need it. ali_bush
	# %patch34 -p0
	# epatch ${WORKDIR}/${P}-bz162177.patch
	# epatch ${WORKDIR}/${P}-ecj-square-bracket-classpath.patch
	# Use ecj for gcj
	# %patch57 -p0
	epatch ${WORKDIR}/eclipse-ecj-gcj.patch
	popd >/dev/null

	# No longer in fedora patchs (i believe) ali_bush
	# %patch38 -p0
	...
	# %patch49 -p0
	epatch ${WORKDIR}/eclipse-ecj-rpmdebuginfo.patch
	...
	# %patch53
	epatch ${WORKDIR}/eclipse-pde.build-add-package-build.patch
	...
	# ali_bush: still in fedora patch set.
	...
	sed --in-place "s:JAVA_HOME = ~/vm/sun142:JAVA_HOME=${JAVA_HOME}:" \
