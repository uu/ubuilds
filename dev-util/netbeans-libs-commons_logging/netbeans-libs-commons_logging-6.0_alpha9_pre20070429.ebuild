...
src_compile() {
	ANT_TASKS="netbeans-nbbuild-6.0 ant-nodeps" eant -f libs/commons_logging/build.xml netbeans -Ddo-not-rebuild-clusters=true
}

src_install() {
	java-pkg_dojar nbbuild/netbeans/extra/modules/org-netbeans-libs-commons_logging.jar
	insinto /usr/share/${PN}-${SLOT}/lib/config
	doins nbbuild/netbeans/extra/config/Modules/org-netbeans-libs-commons_logging.xml
	insinto /usr/share/${PN}-${SLOT}/lib/update_tracking
	doins nbbuild/netbeans/extra/update_tracking/org-netbeans-libs-commons_logging.xml
}
