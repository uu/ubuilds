...
src_unpack() {
	...
	# NEEDS: libs/external/resolver-1_1_nb.jar
}

src_compile() {
	ANT_TASKS="netbeans-nbbuild-6.0 ant-nodeps" eant -f libs/jsr223/build.xml netbeans -Ddo-not-rebuild-clusters=true
}

src_install() {
	java-pkg_dojar nbbuild/netbeans/extra/modules/org-netbeans-libs-jsr223.jar
	insinto /usr/share/${PN}-${SLOT}/lib/config
	doins nbbuild/netbeans/extra/config/Modules/org-netbeans-libs-jsr223.xml
	insinto /usr/share/${PN}-${SLOT}/lib/update_tracking
	doins nbbuild/netbeans/extra/update_tracking/org-netbeans-libs-jsr223.xml
}
