...
src_compile() {
	...
	ejavac -cp $(java-pkg_getjars commons-collections,avalon-framework-4.1,excalibur-instrument-1) -d classes $(find ${SRC_DIR} -name "*.java") || die "Compilation failed"
	jar -cf "${S}/${PN}.jar" -C classes . || die "Could not create jar"
	#Generate javadoc
	if use doc ; then
		mkdir "${JAVADOC_DIR}" || die "Could not create javadoc dir"
		cd ${SRC_DIR}
		javadoc -source "${JAVA_VERSION}" -d ${JAVADOC_DIR} $(find "org/apache/avalon/excalibur/pool" -type d | tr '/' '.') || die "Could not create javadoc"
	fi
}
