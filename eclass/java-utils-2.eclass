...
# @example
#	java-pkg_dojar --virtual dist/jaxmeapi.jar
#
# @param $opt
#	--virtual - Installed jar is also recorded in VIRTUAL_CLASSPATH
...
java-pkg_dojar() {
	...
	while [[ "${1}" == --* ]]; do
		if [[ "${1}" = "--virtual" ]]; then
			...
			die "java-pkg_dojar called with unknown parameter: ${1}"
		fi
		shift
	done
	...
	for jar in "${@}"; do
		...
		if [[ -e "${jar}" ]] ; then
			...
			if [[ ! -L "${jar}" ]] ; then
				...
				if [[ -n "${virtual}" ]]; then
					java-pkg_append_ JAVA_PKG_VIRTUAL_CLASSPATH "${JAVA_PKG_JARDEST}/${jar_basename}"
				fi
...
#
# @param $opt
#	--virtual Recorded jar is also recorded in VIRTUAL_CLASSPATH
...
java-pkg_regjar() {
	...
	while [[ "${1}" == --* ]]; do
		if [[ "${1}" = "--virtual" ]]; then
			...
			die "java-pkg_regjar called with unknown parameter: ${1}"
		fi
		shift
	done
	...
	for jar in "${@}"; do
		...
		if [[ -e "${jar}" || -e "${D}${jar}" ]]; then
			...
			if [[ -n "${virtual}" ]]; then
				java-pkg_append_ JAVA_PKG_VIRTUAL_CLASSPATH "${jar#${D}}"
			fi
		else
			if [[ ${jar} = *\** ]]; then
...
java-pkg_newjar() {
	...
	while [[ "${1}" == --* ]]; do
		if [[ "${1}" = "--virtual" ]]; then
			...
			die "java-pkg_newjar called with unknown parameter: ${1}"
		fi
		shift
	done
	...
	if [[ -n "${virtual}" ]]; then
		java-pkg_dojar --virtual "${new_jar_dest}"
	else
...
# TODO add example
# @param $opt
#	--virtual value is also appended to JAVA_PKG_VIRTUAL_CLASSPATH
...
java-pkg_addcp() {
	...
	while [[ "${1}" == --* ]]; do
		if [[ "${1}" = "--virtual" ]]; then
			...
			die "java-pkg_addcp called with unknown parameter: ${1}"
		fi
		shift
	done
	java-pkg_append_ JAVA_PKG_CLASSPATH "${@}"
	if [[ -n "${virtual}" ]]; then
		java-pkg_append_ JAVA_PKG_VIRTUAL_CLASSPATH "${@}"
	fi
	java-pkg_do_write_
}
...
java-pkg_do_write_() {
	...
	if [[ -n "${JAVA_PKG_CLASSPATH}" || -n "${JAVA_PKG_LIBRARY}" || -n "${JAVA_PKG_VIRTUAL CLASSPATH}" || -f \
		...
		(
			...
			[[ -n "${JAVA_PKG_VIRTUAL_CLASSPATH}" ]] && echo "VIRTUAL_CLASSPATH=\"${JAVA_PKG_VIRTUAL_CLASSPATH}\""
