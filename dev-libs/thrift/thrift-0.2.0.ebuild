...
inherit distutils eutils java-pkg-opt-2 java-ant-2 perl-module
...
src_compile() {
	...
	if use java; then
		cd "${S}"/lib/java || die
		eant ${EANT_EXTRA_ARGS} ${EANT_BUILD_TARGET} \
			$(use_doc ${EANT_DOC_TARGET})
	fi
}
