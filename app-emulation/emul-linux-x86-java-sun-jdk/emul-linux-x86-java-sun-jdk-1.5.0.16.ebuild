...
# $Header: /var/cvsroot/gentoo-x86/app-emulation/emul-linux-x86-java/emul-linux-x86-java-1.5.0.15.ebuild,v 1.8 2009/09/16 20:09:04 serkan Exp $
...
# the app-emulation/emul-linux-x86-java package already has all the logic it needs
# to do a full on JDK, but the Java devs have decided to only give us the JRE..
...
src_install() {
	dodir /opt/${P}
	cp -pPR bin include jre lib man "${D}/opt/${P}/" || die "failed to copy"

	dodoc THIRDPARTYLICENSEREADME.txt || die
